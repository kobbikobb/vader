...
**Hooks** (`hooks/`): `session-start.sh` fires on SessionStart to warn if not in bypass-permissions mode. `pre-tool-use.sh` fires on PreToolUse for the file-editing tools and Bash, and denies edits (including file-writing Bash commands) while the plan frontmatter says `status: planned` — the window between `/vader` saving a plan and `/vader:exec` starting it. Hooks still run under `--dangerously-skip-permissions`, which is why the gate lives here and not in skill `allowed-tools`.
