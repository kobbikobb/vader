...
Between saving a plan and starting it, a `PreToolUse` hook denies the file-editing tools, and Bash commands that write files, so a session cannot skip the loop and start implementing by hand. Read-only Bash still runs. `/vader:exec` lifts it; `/vader:cancel` drops the plan. Claude Code only — the Cursor adapter ships commands and agents, not hooks.
